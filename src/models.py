...
from src.rotations import quat_normalize, quat_relative
...
class RotationCodeHead(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int, n_layers: int = 2):
        ...
        for _ in range(max(n_layers - 1, 0)):
            ...
            dim_in = hidden_dim
        layers.append(nn.Linear(dim_in, 4))
        self.net = nn.Sequential(*layers)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return quat_normalize(self.net(x))
...
class EWPInterpol(nn.Module):
    def __init__(self, args, num_classes=55):
        ...
        pose_hidden_dim = args.pose_mlp_hidden_dim if args.pose_mlp_hidden_dim > 0 else self.equi_emb_size
        self.pose_head = RotationCodeHead(self.equi_emb_size, pose_hidden_dim, n_layers=args.pose_mlp_layers)
        self.pose_head_ema = RotationCodeHead(self.equi_emb_size, pose_hidden_dim, n_layers=args.pose_mlp_layers)
        self.pose_head_ema.load_state_dict(self.pose_head.state_dict())
        self.pose_head_ema.requires_grad_(False)
        print("Invariant projector dims: ", mlp_inv)
        print("Equivariant projector dims: ", mlp_equi)
        print("Pose head hidden dim: ", pose_hidden_dim)
    ...
    def _predict_with_delta(self, representation: torch.Tensor, quat_delta: torch.Tensor) -> torch.Tensor:
        if self.args.predictor_type == "hypernetwork":
            return self.predictor(representation.unsqueeze(1), quat_delta)
        return self.predictor(representation, quat_delta)

    @torch.no_grad()
    def update_pose_head_ema(self) -> None:
        if not self.args.pose_ema:
            return
        momentum = self.args.pose_ema_momentum
        for ema_param, param in zip(self.pose_head_ema.parameters(), self.pose_head.parameters()):
            ema_param.mul_(momentum).add_(param, alpha=1.0 - momentum)

    def _predict_pose(self, representation: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor | None]:
        q_student = self.pose_head(representation)
        if not self.args.pose_ema:
            return q_student, q_student, None

        with torch.no_grad():
            q_ema = self.pose_head_ema(representation)

        # Forward uses the EMA pose while gradients still train the student pose head.
        q = q_student + (q_ema - q_student).detach()
        return q, q_student, q_ema
    ...
    def forward(self, x, y, z, labels):
        ...
        x_all = torch.cat((x_inv, x_equi), dim=1)
        y_all = torch.cat((y_inv, y_equi), dim=1)

        loss_eval, stats_eval = self.evaluator(
            [x_emb.detach(), y_emb.detach()],
            [x_all.detach(), y_all.detach()],
            labels,
            z,
        )
        ...
        stats = {}
        with torch.no_grad():
            stats = std_losses(stats, self.args, "_view1", x_emb, proj_out=x_all)
            stats = std_losses(stats, self.args, "_view2", y_emb, proj_out=y_all)

        repr_loss_inv = F.mse_loss(x_inv, y_inv)

        q_x, q_x_student, q_x_ema = self._predict_pose(x_equi)
        q_y, q_y_student, q_y_ema = self._predict_pose(y_equi)
        q_xy = quat_relative(q_x, q_y)
        q_yx = quat_relative(q_y, q_x)

        y_equi_pred = self._predict_with_delta(x_equi, q_xy)
        x_equi_pred = self._predict_with_delta(y_equi, q_yx)

        endpoint_loss = 0.5 * (
            normalized_mse(y_equi_pred, y_equi.detach())
            + normalized_mse(x_equi_pred, x_equi.detach())
        )

        repr_loss_equi = endpoint_loss

        endpoint_preds = torch.cat(
            FullGatherLayer.apply(torch.cat([y_equi_pred, x_equi_pred], dim=0)),
            dim=0,
        )
        endpoint_preds = endpoint_preds - endpoint_preds.mean(dim=0)
        std_preds = torch.sqrt(endpoint_preds.var(dim=0) + 0.0001)
        pred_std_loss = torch.mean(F.relu(1 - std_preds)) / 2

        x_all_g = torch.cat(FullGatherLayer.apply(x_all), dim=0)
        y_all_g = torch.cat(FullGatherLayer.apply(y_all), dim=0)
        x_all_g = x_all_g - x_all_g.mean(dim=0)
        y_all_g = y_all_g - y_all_g.mean(dim=0)

        std_x = torch.sqrt(x_all_g.var(dim=0) + 0.0001)
        std_y = torch.sqrt(y_all_g.var(dim=0) + 0.0001)
        std_loss = torch.mean(F.relu(1 - std_x)) / 2 + torch.mean(F.relu(1 - std_y)) / 2

        cov_x = (x_all_g.T @ x_all_g) / (self.args.batch_size - 1)
        cov_y = (y_all_g.T @ y_all_g) / (self.args.batch_size - 1)
        cov_loss = off_diagonal(cov_x).pow_(2).sum().div(cov_x.shape[0]) + off_diagonal(cov_y).pow_(2).sum().div(cov_x.shape[0])
        ...
        with torch.no_grad():
            stats = std_losses(stats, self.args, "_pred_xy", y_equi_pred)
            stats = std_losses(stats, self.args, "_pred_yx", x_equi_pred)
            stats["Latent/qx_var"] = q_x.var(dim=0).mean()
            stats["Latent/qy_var"] = q_y.var(dim=0).mean()
            stats["Latent/qdelta_var"] = q_xy.var(dim=0).mean()
            if self.args.pose_ema and q_x_ema is not None and q_y_ema is not None:
                stats["Latent/pose_ema_momentum"] = torch.tensor(
                    self.args.pose_ema_momentum, device=q_x.device, dtype=q_x.dtype
                )
                qx_align = torch.abs((q_x_student.detach() * q_x_ema).sum(dim=1)).mean()
                qy_align = torch.abs((q_y_student.detach() * q_y_ema).sum(dim=1)).mean()
                stats["Latent/pose_ema_align"] = 0.5 * (qx_align + qy_align)
            stats["Latent/raw_endpoint_cos"] = F.cosine_similarity(
                F.normalize(y_equi_pred, dim=1), F.normalize(y_equi.detach(), dim=1), dim=1
            ).mean()

        stats["repr_loss_inv"] = repr_loss_inv
        stats["repr_loss_equi"] = repr_loss_equi
        stats["endpoint_loss"] = endpoint_loss
        stats["std_loss"] = std_loss
        stats["pred_std_loss"] = pred_std_loss
        stats["cov_loss"] = cov_loss
        stats["loss"] = loss
