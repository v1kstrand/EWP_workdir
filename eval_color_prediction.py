...
parser.add_argument("--comet-project-name", type=str, default="sie_3die_eval")
...
class Dataset3DIEBench(Dataset):
    ...
    def __getitem__(self, i):
        ...
        sample_root = self.dataset_root / self.samples[i][1:]
        img_1 = self.get_img(sample_root / f"image_{views[0]}.jpg")
        img_2 = self.get_img(sample_root / f"image_{views[1]}.jpg")         
    
        latent_1 = np.load(sample_root / f"latent_{views[0]}.npy").astype(np.float32)[[3,6]]
        latent_2 = np.load(sample_root / f"latent_{views[1]}.npy").astype(np.float32)[[3,6]]
...
for epoch in range(start_epoch,epochs):
    ...
    for local_step, (inputs_1,inputs_2, latents) in enumerate(train_bar):
        ...
        if step%args.log_freq_time == 0:
            tracker.log_metrics(
                {
                    'Loss/loss': float(loss.item()),
                    'Metrics/train_R2': float(r2.item()),
                    'General/lr': float(args.lr),
                },
                step=step,
            )
        ...
        loss.backward()
        optimizer.step()
        train_bar.set_postfix(loss=f"{loss.item():.4f}", r2=f"{r2.item():.3f}")
    train_bar.close()
    net.eval()
    with torch.no_grad():
        avg_mse = 0
        ...
        for i, (inputs_1,inputs_2, latents) in enumerate(val_bar):
            inputs_1 = inputs_1.to(args.device)
            inputs_2 = inputs_2.to(args.device)
            latents = latents.to(args.device)
            ...
            val_bar.set_postfix(mse=f"{mse.item():.4f}")
        val_bar.close()
        r2 = r2_score(total_preds,total_latents)
        tracker.log_metrics(
            {
                'Metrics/val_MSE': float(avg_mse),
                'Metrics/val_R2': float(r2.item()),
            },
            step=step,
        )
