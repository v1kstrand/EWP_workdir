...
for epoch in range(start_epoch,epochs):
    ...
    for local_step, (inputs_1,labels) in enumerate(train_bar):
            ...
            tracker.log_metrics(
                {
                    ...
                    'Metrics/train top-1': float(top_1.item()),
                    'Metrics/train top-5': float(top_1.item()),
        ...
        loss.backward()
        optimizer.step()
        train_bar.set_postfix(loss=f"{loss.item():.4f}", top1=f"{top_1.item():.3f}")
    train_bar.close()
    net.eval()
    with torch.no_grad():
        len_ds = len(ds_val)
        ...
        for i, (inputs_1,labels) in enumerate(val_bar):
            inputs_1 = inputs_1.to(args.device)
            labels = labels.to(args.device)
            ...
            current_top_1, _ = accuracy(outputs, labels, topk=(1, 5))
            val_bar.set_postfix(top1=f"{current_top_1.item():.3f}")
        val_bar.close()
        top_1, top_5 = accuracy(total_preds, total_labels, topk=(1, 5))
        tracker.log_metrics(
            {
                'Metrics/val top-1': float(top_1.item()),
                'Metrics/val top-5': float(top_5.item()),
            },
            step=step,
        )
